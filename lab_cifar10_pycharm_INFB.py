"""
PyCharm Lab Script — INF B (CIFAR-10 + Augmentasi)
==================================================
Fokus: Pengaruh augmentasi (Rotation + Flip + ColorJitter) terhadap SmallCNN
Varian model (dilatih berurutan):
 1) max_flatten
 2) avg_flatten
 3) max_gap
 4) avg_gap
Artefak disimpan ke --outdir (default: runs_cifar10_infb):
 - results.csv
 - logs/<varian>_curves.png
 - confusion_matrices/<varian>_confmat.png
 - feature_maps/<best>_block1.png, <best>_block3.png
 - gradcam/<best>/{correct_*.png, wrong_*.png}
Contoh pemakaian:
- CPU (pasti jalan): python lab_cifar10_pycharm_INFB.py --epochs 4 --batch_size
128 --device cpu
- Otomatis (CUDA/MPS/CPU): python lab_cifar10_pycharm_INFB.py --epochs 4 --
batch_size 128 --device auto
- NVIDIA CUDA (jika ada): python lab_cifar10_pycharm_INFB.py --device cuda
- Apple Silicon (M1/M2/M3): python lab_cifar10_pycharm_INFB.py --device mps
"""
...
class SmallCNN(nn.Module):
    def __init__(self, num_classes=10, pool='max', head='flatten'):
        super().__init__()
        self.block1 = ConvBlock(3, 32, pool)   # 32->16
        self.block2 = ConvBlock(32, 64, pool)  # 16->8
        self.block3 = ConvBlock(64, 128, pool) # 8->4
        self.head_type = head
...
# ---------------- Data loaders (dengan augmentasi wajib INF B) ----------------
def build_loaders(batch_size: int, workers: int = 2, with_extra_aug: bool = True):
    # Statistik standar CIFAR-10
    mean, std = [0.4914, 0.4822, 0.4465], [0.2023, 0.1994, 0.2010]

    # Augmentasi wajib untuk INF B
    aug_list = []
    if with_extra_aug:
        aug_list += [
...
def train_epochs(model, train_ld, test_ld, device, epochs=4, lr=3e-4, tag="model", outdir="runs_cifar10_infb"):
...
def save_feature_maps(model, loader, device, outdir, tag="model", layer_name='block1', n_ch=8):
    ...
    handle = getattr(model, layer_name).register_forward_hook(
        lambda m, i, o: feats.setdefault('f', o.detach().cpu())
    )
    _ = model(x)
    handle.remove()
...
def overlay_cam(img_tensor, cam, mean, std):
    ...
    heat = cam.squeeze(0).unsqueeze(0).unsqueeze(0)   # (1,1,H,W)
    heat = F.interpolate(heat, size=(32,32), mode="bilinear", align_corners=False)
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="runs_cifar10_infb")
    parser.add_argument("--epochs", type=int, default=4)
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--workers", type=int, default=2)
    parser.add_argument("--device", type=str, default="auto", choices=["auto", "cpu", "cuda", "mps"])
    parser.add_argument("--no_extra_aug", action="store_true",
                        help="Matikan augmentasi tambahan (Rotation + ColorJitter) jika diperlukan.")
    ...
    train_ld, test_ld, classes = build_loaders(
        batch_size=args.batch_size,
        workers=args.workers,
        with_extra_aug=(not args.no_extra_aug)
    )
    ...
    for name, cfg in variants:
        ...
        model = train_epochs(model, train_ld, test_ld, device, epochs=args.epochs,
                             lr=args.lr, tag=name, outdir=args.outdir)
    ...
    # Feature maps & Grad-CAM untuk model terbaik (berdasarkan akurasi)
